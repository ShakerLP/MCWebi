   ...
   if (map.get("action").equals("status")) {
    if (hasPermissions(DebugServer.logged.get(session.getCookies().read("session")), session.getCookies().read("server"), "status")) {
     if (session.getCookies().read("server") == null) {
      ...
      msg += "</div><div class='col-sm-8'><h1>Status</h1><div class='col-sm-3'>" + "<h4>TPS</h4>" + round(Lag.getTPS(), 3, BigDecimal.ROUND_HALF_UP) + " TPS" + "<progress class='progress' value='" + Lag.getTPS() + "' max='20'></progress>";
      ...
      msg += "</div><div class='col-sm-8'><h1>Status</h1><div class='col-sm-3'>" + "<h4>TPS</h4>" + round(json.getDouble("tps"), 3, BigDecimal.ROUND_HALF_UP) + " TPS" + "<progress class='progress' value='" + Lag.getTPS() + "' max='20'></progress>";
      msg += "</div><div class='col-sm-3'>";
     ...
     if (session.getCookies().read("server") == null) {
      if (map.containsKey("mode")) {
       if (map.get("mode").equals("edit")) {
        ...
        msg += "<form method='POST' action='?action=filebrowser&mode=save&path=" + map.get("path") + "&file=" + file.getPath() + "'><div class='form-group'><textarea class='form-control' rows='25' name='text' id='file'>" + readFile(file) + "</textarea></div><input type='submit' value='Save' class='btn btn-success'></form><br>";
       }
      ...
      for (String s: Config.data.getStringList("servers")) {
       if (s.contains(session.getCookies().read("server"))) {
        ...
        try {
         ...
         msg += json.getString("dir").replace("{icon_file}", icon.file).replace("{icon_back}", icon.back).replace("{icon_folder}", icon.folder).replace("{icon_remove}", icon.remove).replace("{icon_edit}", icon.edit);
        } catch (JSONException | IOException e) {}
       }
      }
     }
